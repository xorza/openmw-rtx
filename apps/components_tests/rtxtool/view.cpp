    ...
    namespace
    {
        ...
        TEST(RtxProfileLineTest, everyConditionTheFrameDependsOnIsInTheLine)
        {
            ...
            EXPECT_EQ(describeProfile(request, validation, origin, target, 2560, 1440),
                ...
                " --fov=60 --size=2560x1440 --weather=Ashstorm --hour=17.25 --validation=true --sync-validation=true"
        ...
        /// Both of the fields that are a flag rather than a value, since a line that dropped either
        /// would reproduce a different frame while looking correct.
